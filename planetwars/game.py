...
def _neutral_player(turn, pid, planets, fleets, train=False):
    return []
...
class PlanetWars:
    ...
    def __init__(self, players, map_name=None, planets=None, fleets=None,
                                turns_per_second=None, turn=0, collisions=False, train=False):
        if len(players) < 2:
    ...
    def play(self):
        ...
        while winner < 0:
            ...
            # Do the turn
            early_stop = self.do_turn()
            # Update views
            planets, fleets = self.freeze()
            for view in self.views:
                view.update(planets, fleets)
            # Check for end game.
            winner, ship_counts, turns = self.gameover(early_stop)
    ...
    def do_turn(self):
        ...
        for i, player in enumerate(self.players):
          prev = time.time()
          player_orders.append(player(self.turn, i, planets, fleets, self.train))
          t = time.time()-prev
...
#   security issue and is disqualified.
    ...
    def gameover(self, early_stop = False):
        if early_stop:
            return 3, [], self.turn
